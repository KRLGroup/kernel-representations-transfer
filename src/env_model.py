...
def getEnvModel(env, obs_space):
    ...
    if isinstance(env, ToyMCEnv):
        return ToyMcEnvModel(obs_space)
...
class ToyMcEnvModel(EnvModel):
    def __init__(self, obs_space):
        super().__init__(obs_space)
        self.embedding_size = 2 # coordinates

    def forward(self, obs):
        if "features" in obs.keys():
            # TODO pass through some kind of model?
            return obs.features.float()
